...
public class PlanController {
    ...
    public void initialize() {
        ...
        // �couteur pour la s�lection d'un objet dans la ComboBox
        ...
        sauvegarderPlanButton.setOnAction(event -> {
            ...
            fileChooser.setTitle("S�lectionner un emplacement pour la sauvegarde");
            fileChooser.getExtensionFilters().add(new FileChooser.ExtensionFilter("Fichier JSON (*.json)", "*.json"));

            // Ouvrir le menu de sauvegarde et attendre que l'utilisateur effectue une s�lection
            File selectedFile = fileChooser.showSaveDialog(etage.getScene().getWindow());

            if (selectedFile != null) {
                String filePath = selectedFile.getParent(); // R�cup�rer le chemin du dossier
                String fileName = selectedFile.getName(); // R�cup�rer le nom du fichier
        ...
        ouvrirPlanButton.setOnAction(event -> {
            ...
            // Afficher la bo�te de dialogue d'ouverture de fichier
